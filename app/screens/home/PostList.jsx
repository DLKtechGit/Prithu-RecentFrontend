...
const PostList = forwardRef<PostListHandle, PostListProps>(
  ({ scrollRef, categoryId, sheetRef, optionSheet }, ref) => {
    ...
    const fetchPosts = async (catId | null = null) => {
      try {
        setLoading(true);
    ...
    const recordViewCount = async (feedId) => {
      try {
    ...
    const handleBoxLayout = (id) => (event: any) => {
    ...
    const handleDislikeUpdate = (postId, newIsDisliked: boolean, newDislikeCount: number) => {
      setPosts((prevPosts) =>
        prevPosts.map((p) =>
          p._id === postId
